...
function SignInPage() {
  ...
  const handleSignIn = async (roleToSet: RoleId = selectedRole) => {
    setIsLoading(true);
    setGeneralError("");
    try {
      await loginWithRole(roleToSet);
      navigate({ to: roleTargetMap[roleToSet] });
    } catch (err) {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!selectedRole) {
      setGeneralError("Please select a user role to continue.");
      hasError = true;
    }
  ...
  const handleSSOClick = async () => {
    setInfoNotice("SSO authentication verified. Logging in with corporate credentials...");
    setIsLoading(true);
    try {
      await loginWithRole(selectedRole);
      navigate({ to: roleTargetMap[selectedRole] });
    } catch {
      setGeneralError("SSO authentication failed.");
    } finally {
      setIsLoading(false);
    }
  };
              ...
              <div className="mx-auto w-full max-w-md">
                ...
                <div className="hidden items-center justify-between gap-3 border-b border-border pb-5 lg:flex">
                  <div>
                    ...
                    <Mono className="mt-1 text-muted-foreground">Digital Investigator</Mono>
                  </div>
                  <Link
                    to="/"
                    className="text-sm font-medium text-muted-foreground transition-colors hover:text-foreground"
                ...
                {infoNotice && (
                  <div className="mt-4 flex items-center gap-2 rounded-xl border border-violet/30 bg-violet/10 p-3 text-xs text-violet">
                    <Info className="size-4 flex-shrink-0" aria-hidden />
                ...
                <div className="mt-6">
                  <RoleSelector
                    ...
                    onContinue={() => handleSignIn(selectedRole)}
                  />
                </div>
                ...
                <div className="mt-6 rounded-2xl border border-border bg-offwhite p-4">
                  <div className="flex items-start gap-3">
                    <ShieldCheck className="mt-0.5 size-4 text-violet" aria-hidden />
                    <div>
                      <p className="text-sm font-medium text-foreground">
                        Protected investigation environment
                      </p>
                      <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
                        Session activity is monitored and audit-ready. Server-side
